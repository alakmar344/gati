      ...
      <div className="glass-panel max-w-2xl mx-auto rounded-2xl p-2.5 animate-rise">
        <form onSubmit={handleSubmit} className="flex items-center gap-2">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none text-slate-900 placeholder:text-slate-400 text-sm font-mono font-medium focus:outline-none px-1 uppercase"
          />
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        ...
        {sampleRefs.map((s) => (
          <button
            ...
            className="px-3 py-1.5 rounded-full bg-white hover:bg-slate-50 border border-slate-200 hover:border-emerald-300 text-slate-600 hover:text-slate-900 font-mono text-[11px] font-semibold transition-colors"
            title={s.label}
          >
            {s.ref}
          </button>
        ...
        <div className="card p-6 sm:p-10 space-y-8 animate-rise">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 hairline border-b">
            <div className="space-y-2">
              ...
              <div className="text-sm text-slate-500">
                Applicant <strong className="text-slate-800 font-semibold">{activeApp.applicantName}</strong>
                {' '}&middot; RTO <strong className="text-slate-800 font-semibold">{activeApp.rtoName}</strong>
              </div>
            </div>

            <div className="text-left sm:text-right shrink-0">
              <span className="eyebrow text-slate-400 block mb-1.5">Current State</span>
              <span className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-full">
          ...
          <div className="p-5 rounded-2xl bg-emerald-50/70 border border-emerald-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1.5">
              ...
              <p className="text-xs leading-relaxed text-emerald-800 max-w-xl">
          ...
          <div className="space-y-5">
            <h3 className="eyebrow text-slate-400">Application Milestones</h3>

            <div className="relative">
              {/* Connecting rail */}
              <div className="absolute left-[11px] top-2 bottom-2 w-px bg-slate-200" aria-hidden="true" />
                  ...
                  return (
                    <li key={idx} className="relative flex gap-4">
                      ...
                      <div className="flex-1 min-w-0 pb-1">
                        ...
                        <p className="text-xs leading-relaxed text-slate-500 mt-1">{step.description}</p>
                      </div>
                    </li>
                  );
                })}
          ...
          {activeApp.payment && (
            <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 flex flex-wrap items-center justify-between gap-4">
              <div className="space-y-1">
                <span className="eyebrow text-slate-400 block">Payment Reference</span>
                <div className="font-mono font-bold text-slate-900 text-sm">{activeApp.payment.transactionId}</div>
                <div className="text-[11px] text-slate-500 font-mono">Bank UTR: {activeApp.payment.utrNumber}</div>
              </div>

              <div className="text-right">
                <span className="eyebrow text-slate-400 block">Settled Amount</span>
        ...
        <div className="card p-12 text-center space-y-4 animate-rise">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-sm text-slate-500 max-w-sm mx-auto leading-relaxed">
              ...
              <span className="font-mono font-semibold text-slate-700">{searchRef}</span>. Double-check the
