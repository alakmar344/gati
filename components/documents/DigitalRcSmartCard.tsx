    ...
    <div className="flex flex-col items-center gap-6 w-full max-w-xl mx-auto">
      ...
      <div 
        className={`w-full aspect-[1.586/1] rounded-3xl p-6 sm:p-7 relative overflow-hidden shadow-2xl transition-all duration-300 border ${
          isEV
            ? 'bg-gradient-to-br from-[#064e3b] via-[#065f46] to-[#047857] text-white border-olive-400/40 shadow-olive-950/20'
            : 'bg-gradient-to-br from-[#1a2014] via-[#28301e] to-[#444c28] text-white border-white/20 shadow-olive-950/30'
        }`}
        style={{
      ...
      >
        ...
        <div className="flex items-start justify-between relative z-10 border-b border-white/15 pb-3">
          ...
          <div className="text-right">
            <div className="text-[9px] uppercase tracking-widest text-olive-500/80">SMART CARD UID</div>
        ...
        <div className="flex items-center justify-between my-3 sm:my-4 relative z-10">
          ...
          <div className="px-4 py-1.5 rounded-xl bg-black/40 backdrop-blur-md border border-white/20 shadow-inner">
            <span className="text-xs text-olive-400 mr-2 uppercase tracking-wider">REGN NO:</span>
        ...
        <div className="grid grid-cols-12 gap-2 sm:gap-3 text-xs relative z-10">
          <div className="col-span-8 space-y-1">
            <div>
              <div className="text-[9px] uppercase tracking-wider text-olive-400">Registered Owner</div>
              <div className="font-bold text-sm tracking-wide text-white truncate">{rc.ownerName}</div>
            </div>
            <div className="grid grid-cols-2 gap-2 pt-0.5">
              <div>
                <div className="text-[8px] uppercase tracking-wider text-olive-400">Make / Model</div>
                <div className="font-semibold text-[11px] text-olive-100 truncate">{rc.modelName}</div>
              </div>
              <div>
                <div className="text-[8px] uppercase tracking-wider text-olive-400">Fuel & Power</div>
                <div className="font-semibold text-[11px] text-olive-300 truncate">
            ...
            <div className="grid grid-cols-2 gap-2 pt-0.5">
              <div>
                <div className="text-[8px] uppercase tracking-wider text-olive-400">Issue Date</div>
                <div className="font-mono text-[10px] text-olive-200">{rc.issueDate}</div>
              </div>
              <div>
                <div className="text-[8px] uppercase tracking-wider text-olive-400">Valid Upto</div>
                <div className="font-mono text-[10px] text-olive-300 font-bold">{rc.validUpto}</div>
              </div>
            </div>
          </div>

          {/* QR Code & Security Stamp */}
          <div className="col-span-4 flex flex-col items-center justify-center bg-white/90 rounded-2xl p-2 text-olive-950 shadow-inner">
            <QrCode className="w-14 h-14 sm:w-16 sm:h-16 text-olive-950" />
            <span className="text-[8px] font-mono font-bold text-olive-800 tracking-tighter mt-1">
              DIGITAL VERIFIED
            </span>
          </div>
        </div>
        ...
        <div className="absolute bottom-2 left-6 right-6 flex items-center justify-between text-[8px] text-white/50 border-t border-white/10 pt-1.5">
          ...
          <span className="font-semibold text-olive-300">DEMO • NOT AN OFFICIAL GOVERNMENT DOCUMENT</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 w-full justify-center">
        ...
        <button
          onClick={handlePrint}
          className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-olive-600 hover:bg-olive-700 text-white text-xs font-semibold shadow-md shadow-olive-600/20 transition-all"
        >
